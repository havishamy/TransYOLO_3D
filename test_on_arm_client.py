...
class Config:
    ...
    # 相机内参（需通过标定获取，示例值）
    CAMERA_INTRINSIC = np.array([[604.0, 0, 334.7], [0, 603.7, 250.7], [0, 0, 1]])
...
class RealTimeTriangulateAndApproach:
    def __init__(self, args):
        ...
        # 2. 初始化远程YOLO客户端
        self.K = np.array([[args.Kfx, 0, args.Kcx], [0, args.Kfy, args.Kcy], [0, 0, 1]], dtype=np.float64)
        ...
        self.yolo_client = RemoteYOLOClient(
            args.yolo_server_ip, args.yolo_server_port, self.base_frame, self.camera_frame, self.K
        )
        ...
        try:
            ...
            if len(dets) > 0:
                ...
                save_path = os.path.join(
                    "/home/dsj/ur5e/src/universal_robot/ur5e_moveit_config/scripts",
                    f"detection_{timestamp}_{len(dets)}.jpg",
                )
                # 保存图像
            ...
            rospy.logerr(f"采集/识别/可视化失败：{e!s}")
            # 异常时关闭窗口，避免资源泄漏
            cv2.destroyAllWindows()
            return [], None
    ...
    def _compute_3d_position(self, rays):
        ...
        # 最小化距离之和
        result = minimize(fun=distance_to_rays, x0=initial_guess, args=(rays,), method="L-BFGS-B")
    ...
    def collect_views_for_object(self, target_class_id=None):
        """Move the arm slightly N times and capture detection + camera poses. Returns list of (C_base (3,), d_base
        (3,)) pairs for the selected tracked object (we pick highest confidence detections).
        """
        # Move pattern: small lateral steps around current pose (robot stays safe)
        self.group.get_current_pose(self.group.get_end_effector_link()).pose
            ...
            for i in range(15):
                ...
                if plan[0]:
                    ...
                    for j in range(1, len(waypoints)):
                        # 简单计算关节空间距离
                        diff = sum(
                            (waypoints[j].positions[k] - waypoints[j - 1].positions[k]) ** 2
...
def parse_args():
    ...
    p.add_argument(
        "--model",
        default="/home/dsj/ur_ws/src/ur5e_robot/manipulator/scripts/ultralytics/runs/train/yolov11_glassware/weights/best.pt",
    )
