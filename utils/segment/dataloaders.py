    ...
    def __getitem__(self, index):
        ...
        if nl:
            ...
            if self.overlap:
                ...
                edges = edges[None]  # (640, 640) -> (1, 640, 640)
                labels = labels[sorted_idx]
            else:
                masks, edges = polygons2masks(img.shape[:2], segments, color=1, downsample_ratio=self.downsample_ratio)
        ...
        return (torch.from_numpy(img), labels_out, self.im_files[index], shapes, masks, edges)
...
def save_edge_map(edge_map, save_path="/home/dsj/code/yolov5_headseg/edge_vis.png"):
    ...
    # 转到 CPU，转 numpy，保证 float
    edge_map = edge_map.cpu().numpy().astype(np.float32)
...
def polygons2masks(img_size, polygons, color, downsample_ratio=1):
    ...
    for si in range(len(polygons)):
        mask, edge = polygon2mask(img_size, [polygons[si].reshape(-1)], color, downsample_ratio)
        masks.append(mask)
        edges.append(edge)
    return np.array(masks), np.array(edges)
